...
void animate(void)
{
	if (play) {
		...
		if (direction_right && !stop) {
			movKit_z += 0.4f; //para mover o detener el carro_ iniciar i detener la animacion
		}
		if (!direction_right && !stop){
			movKit_z -= 0.4f; //para mover o detener el carro_ iniciar i detener la animacion
		}
...
void display(Shader shader, Model carril, Model personaje)//ahora si resiven parametros: shader, carril y personaje
{
	shader.use();
	...
	tmp = model = glm::translate(model, glm::vec3(15.0f, -1.75f, 0.0f));
	model = glm::scale(model, glm::vec3(0.055f, 0.07f, 0.02f));
	shader.setMat4("model", model);
	carril.Draw(shader);//para dibujar el modelo del personaje

	model = glm::translate(tmp, glm::vec3(movKit_x, 0.03f, movKit_z));
	model = glm::scale(model, glm::vec3(0.07f, 0.08f, 0.08f));
...
int main()
{
	...
	// Load models
	Model carril =((char *)"Models/carril.obj");//para cargar el modelo del carril
...
//ya que el pivote esta mal ubicado en el modelo de la llanta
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		display(modelShader, carril, personaje);
