...
void display(void)
{
	...
	glm::mat4 modeltemp = glm::mat4(1.0f); //temporal utilizado para dibujar robot sin los brazos
	...
	//brazo derecho
	//hombro 
	model = glm::translate(modeltemp, glm::vec3(2.5, 2.0, 0.0));//model temp reinicializa la matriz al centro del cuerpo
	modelTemp = model;
	...
	*/
	...
	//brazo izquierdo
	//hombro
	model = glm::translate(modeltemp, glm::vec3(-2.5, 2.0, 0.0));
	modelTemp = model;
	model = glm::rotate(model, glm::radians(-angHom), glm::vec3(0, 0, 1));/*se cambio el segino del angulo para que
	realice el movimiento opuesto al brazo derecho, esto porque, si no se hace asi, el brazo izquierdo hace los
	movimiento hacia atras, cosa que no deberia de ser posible, "dobla el brazo hacia atras del cuerpo". Esto se 
	hace con todos los angulos para obtener el movimiento correcto*/
	projectionShader.setMat4("model", model);
	projectionShader.setVec3("aColor", glm::vec3(1, 1, 0));
	glDrawArrays(GL_QUADS, 0, 24);
